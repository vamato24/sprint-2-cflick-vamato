...
test("csvLoader correct loads affects active", function () {
...
test("csvLoader incorrect loads doesn't unload previous", function () {
    expect(main.csvLoader("/test/dataOne.csv")).toBe(true);
    expect(main.csvLoader("dhjscv sefwef")).toBe(false);
    expect(main.returnActiveData()).toEqual([
        ["1", "2", "3"],
...
test("parseCommand correct commands", function () {
    ...
    expect(main.parseCommandCall("search 2")).toEqual([
        ["1", "2", "3"],
        ["a", "b", "c"],
        ["true", "false", "3"],
        ["3", "6", "9"],
    ]);
    expect(main.parseCommandCall("search 2 3")).toEqual([
        ["1", "2", "3"],
        ["true", "false", "3"],
    ]);
});
test("parseCommand incorrect commands", function () {
    expect(function () {
        main.parseCommandCall("search");
    }).toThrow();
...
test("csvSearcher Out Of Bounds", function () {
    main.csvLoader("/test/dataOne.csv");
    expect(function () {
        main.csvSearcher("-1", "owef");
    }).toThrow();
    expect(function () {
        main.csvSearcher("5", "");
    }).toThrow();
    ...
    expect(function () {
        main.csvSearcher("ewiubqwdq", " ");
    }).toThrow();
    expect(function () {
        main.csvSearcher("some words", " ");
    }).toThrow();
    expect(function () {
        main.csvSearcher("some2words", " ");
    }).toThrow();
});
test("csvSearcher Search by num index", function () {
    main.csvLoader("/test/dataOne.csv");
    expect(main.csvSearcher("1", "hi")).toEqual([]);
    expect(main.csvSearcher("0", "a")).toEqual([["a", "b", "c"]]);
    expect(main.csvSearcher("2", "3")).toEqual([
        ["1", "2", "3"],
        ["true", "false", "3"],
    ]);
    main.csvLoader("/test/dataSeven.csv");
    expect(main.csvSearcher("0", "hey")).toEqual([
        ["hey", "hi", "hello", "what's up"],
    ]);
    expect(main.csvSearcher("1", "hello")).toEqual([]);
    expect(main.csvSearcher("3", "4th")).toEqual([
        ["four", "4", "fourty4", "4th"],
    ]);
});
test("csvSearcher Search by text header", function () {
    main.csvLoader("/test/dataSeven.csv");
    expect(main.csvSearcher("four", "hey")).toEqual([
        ["hey", "hi", "hello", "what's up"],
    ]);
    expect(main.csvSearcher("4th", "dance")).toEqual([
        ["wioeufn", "weio", "wefuo", "dance"],
    ]);
    //The header being only a number indicates interpretation as an int index, not a text header.
    expect(function () {
        main.csvSearcher("4", "hi");
    }).toThrow();
});
test("csvSearcher no data", function () {
    main.csvLoader("/test/dataSix.csv");
    expect(function () {
        main.csvSearcher("", "");
    }).toThrow();
});
test("csvSearcher empty string", function () {
